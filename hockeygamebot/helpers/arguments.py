...
def _parse_local_arguments(sysargs):
    ...
    parser.add_argument("--notweets", help="log tweets to console instead of Twitter", action="store_true")
    ...
    parser.add_argument("--overridelines", help="override lines if None are returned", action="store_true")
    parser.add_argument("--yesterday", help="get yesterday game on the schedule", action="store_true")
    ...
    parser.add_argument("--config", help="Overrides the config.yaml with another filename.", action="store")
...
def parse_arguments(sysargs=None):
    ...
    if args.docker:
        ...
        if not os.path.exists(DOCKER_CONFIG):
            print("[ERROR] Docker requires a configuration file to be passed into the `docker run` command.")
            print(
